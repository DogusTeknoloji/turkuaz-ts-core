 ...
 * Apply a Backup to localStorage                           function applyBackup(backup: Backup,
 *                                                                               fClear: boolean = true,
...
export function consoleInfo(fShowMaximumSize: boolean = false) {
  ...
  for (let i = 0; i < localStorage.length; ++i) {
    const key = localStorage.key(i);
    const value = localStorage.getItem(key!);
    debugConsole.log(amount, key, value);
    size += key!.length + value!.length;
    amount++;
  }
  debugConsole.log('Total entries:', amount);
  debugConsole.log('Total size:', size);
  if (fShowMaximumSize === true) {
    const maxSize = getMaximumSpace();
    debugConsole.log('Total size:', maxSize);
  }
}
...
/*
    ...
    LocalStorage.consoleInfo()                                      // show all the info we have,
